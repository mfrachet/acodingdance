...
Before getting further about SSR, let's try to see what happens when a user makes a query on a site that is NOT using SSR:

- The browser makes a query on a specific URI
...
We call this way of building an application "Client Side Rendering" (or CSR).
